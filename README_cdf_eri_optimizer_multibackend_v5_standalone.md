...
## Standalone file note

The standalone distribution file is:

```text
cdf_eri_optimizer_multibackend_v5_standalone.py
```

It does **not** import `cdf_eri_optimizer_multibackend_v3.py` or `cdf_eri_optimizer_multibackend_v4.py`. It contains the required v3/v4 helper code inline plus the v5 optimizer layer. You only need normal Python dependencies:

```bash
pip install numpy scipy
pip install torch      # for Torch CPU/CUDA/MPS backend
pip install mlx        # for MLX backend on Apple Silicon
```

Use it in notebooks as:

```python
from cdf_eri_optimizer_multibackend_v5_standalone import CDFERIOptimizer
```
